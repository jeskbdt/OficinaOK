// PERSONALIZE: trilhas, preços e descrições de dificuldade.
const CONFIG = {
  optionField: 'trail', detailField: 'date', maxQuantity: 6, success: 'Experiência reservada na simulação',
  options: {
    bosque: { label: 'Trilha do Bosque', price: 85, info: 'Bosque: 3 km · 2 horas · nível leve.' },
    mirante: { label: 'Caminho do Mirante', price: 120, info: 'Mirante: 6 km · 4 horas · nível moderado.' },
    aguas: { label: 'Rota das Águas', price: 150, info: 'Águas: 8 km · 5 horas · nível moderado.' }
  }
};
...
function updateTrailInfo() {
  const trail = CONFIG.options[form.elements.trail.value];
  document.querySelector('[data-trail-info]').textContent = trail?.info || 'Escolha uma trilha.';
}
form.elements.trail.addEventListener('change', updateTrailInfo);
updateTrailInfo();
